...
const int eyenumber=13;
const int brownumber=10;
const int nosenumber=5;
const int mouthnumber=10;
const int colornumber=5;
const int clonumber=7;
const int backgroundnumber=7;
const int cefanumber=7;
const int houfanumber=13;
const int liuhainumber=6;
const int faceDecnumber=8;
const int headDecnumber=15;
    ...
    , ui(new Ui::Widget)
{
    ...
    bgm->setMedia(QUrl("qrc:/res/BGM.mp3"));//qrc需要标注
    bgm->play();
    soundTimer->start(651000);
...
void Widget::on_colorColorCommitButton_clicked()
{
    ...
    for(int i=0;i<colorImage.width();i++)
    {
     ...
     ui->colorLabel->setPixmap(QPixmap::fromImage(colorImage));//写错位置导致多次触发！造成卡顿！//必须写，否则修改后的不会显示，但图像上的其实已经修改
     colorColorFlag=0;
     colorChoose=0;
    }
    else
...
void Widget::on_commitNameButton_clicked()
{
    ...
    else
    {
        ...
        QMessageBox::information(this,"帮助文档","首先感谢你游玩本游戏!\n\n预警：本游戏拥有并不高的选型自由度和奇怪的绘制比例，且部分部件之间存在着强不适配性，系本人时间不充裕与画工不成熟所致，纯属个人自嗨产物，还请见谅TT\n\n以“后发”为例说明使用方法:\n首先点击“后发”按钮进入选择模式；\n再点击其右侧的“确定”按钮来确定后发的基本样式；\n然后点击“颜色”按钮，进入颜色选择模式，在此模式下，你可以通过在最左边的显示框中点击对应部分上所需要修改的颜色，并选择你所需要的的颜色；\n选定颜色后，点击“颜色”按钮右侧的“确定”按钮，等待一段时间，即可完成该部分对应颜色的更改；\n最后点击“确认生成”按钮，即可完成对象的生成。\n\nTips:\n(1)你可以不选择某部分的样式，或者多次生成同一角色，但是新生成的角色形象会覆盖旧的，还请注意！\n(2)无论你是否进入颜色选择模式，你都可以先不修改本部分颜色而去选择其他部分的样式；你可以通过窗口标题来回忆你是否进入或进入了哪个颜色模式。\n(3)尽管你可以在任何时候进入任何部分的颜色选择模式，但需要注意不同部分之间的叠加关系。虽然你可以点入某一已被覆盖的部分，但这会阻碍你的观察。\n(4)准备将某部分染成rgb均为0的纯黑色时需要注意，这一部分你不能再次染色了；染成除此以外的任何颜色时都不用考虑这一点。\n(5)每次选色的默认值是显示框中你正修改的部分中你所选定的位置上的颜色，你可以据此判断自己是否选入了自己想要修改的区域；你也可以通过将多个部分改成一个颜色以方便后续的一键调色，但这个操作是不可逆的。(6)渐变色的背景不好染色，还请见谅。\n");
    }
}
...
{
    pushButtonSound->play();
    QMessageBox::information(this,"帮助文档","首先感谢你游玩本游戏!\n\n预警：本游戏拥有并不高的选型自由度和奇怪的绘制比例，且部分部件之间存在着强不适配性，系本人时间不充裕与画工不成熟所致，纯属个人自嗨产物，还请见谅TT\n\n以“后发”为例说明使用方法:\n首先点击“后发”按钮进入选择模式；\n再点击其右侧的“确定”按钮来确定后发的基本样式；\n然后点击“颜色”按钮，进入颜色选择模式，在此模式下，你可以通过在最左边的显示框中点击对应部分上所需要修改的颜色，并选择你所需要的的颜色；\n选定颜色后，点击“颜色”按钮右侧的“确定”按钮，等待一段时间，即可完成该部分对应颜色的更改；\n最后点击“确认生成”按钮，即可完成对象的生成。\n\nTips:\n(1)你可以不选择某部分的样式，或者多次生成同一角色，但是新生成的角色形象会覆盖旧的，还请注意！\n(2)无论你是否进入颜色选择模式，你都可以先不修改本部分颜色而去选择其他部分的样式；你可以通过窗口标题来回忆你是否进入或进入了哪个颜色模式。\n(3)尽管你可以在任何时候进入任何部分的颜色选择模式，但需要注意不同部分之间的叠加关系。虽然你可以点入某一已被覆盖的部分，但这会阻碍你的观察。\n(4)准备将某部分染成rgb均为0的纯黑色时需要注意，这一部分你不能再次染色了；染成除此以外的任何颜色时都不用考虑这一点。\n(5)每次选色的默认值是显示框中你正修改的部分中你所选定的位置上的颜色，你可以据此判断自己是否选入了自己想要修改的区域；你也可以通过将多个部分改成一个颜色以方便后续的一键调色，但这个操作是不可逆的。\n(6)渐变色的背景不好染色，还请见谅。\n");
}
